...
extern TIME les_time[845];//时间表头
extern PLACE les_place[260];//空间表头
extern bool readed[845*260];//在查询函数中标记此课堂是否已读
extern std::vector<INFO> gen_info ;//储存综合信息
extern std::vector<CLASSNO> les_classno;//专业班级查找链
extern std::vector<COURSE> les_course;//课程查找链
extern std::vector<int>find;//删除函数中用来储存遍历过程中要删除的数据元素的下标的vector
extern  int lesson_sum;//储存矩阵中的节点个数
...
BOOL MaintainLessonInfo(void);    /*维护课堂信息*/
...
BOOL QueryCourseByTeacher(void);       /*通过教师查询课程*/
...
BOOL StatClassroomRate(void);       /*教室利用率分析*/
